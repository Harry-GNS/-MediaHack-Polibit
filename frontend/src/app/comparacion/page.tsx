          ...
          {grupos.map((grupo, index) => (
            <tr key={`${grupo.ambito}-${index}`} className="align-top hover:bg-white/[.025] transition-colors">
              ...
              {candidatos.map(candidato => {
                ...
                return <td className="py-6 px-6 leading-relaxed text-gray-300 font-light" key={candidato.id}>
                  {items.length ? items.map((item, itemIndex) => <div className="mb-4 last:mb-0" key={`${item.id}-${itemIndex}`}>
                    <p className="text-base">{proposal(item)}</p>
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <section className="mt-16 border-t border-white/10 pt-16 grid gap-16 xl:grid-cols-[1.7fr_1fr]">
        ...
        <aside className="flex flex-col">
          ...
          {respuesta && <div className="mt-16 pt-10 border-t border-white/5"><h3 className="text-[10px] font-mono uppercase tracking-[0.3em] text-accent mb-6">Respuesta basada en evidencia</h3><p className="whitespace-pre-wrap text-2xl font-light leading-relaxed text-white/90 mb-10">{respuesta}</p>{evidencias.length > 0 && <div className="w-full overflow-x-auto"><table className="w-full text-left text-sm border-t border-white/10"><thead className="text-[10px] font-mono uppercase tracking-[0.3em] text-gray-500"><tr><th className="py-6 pr-4 font-normal w-1/4">Candidatura</th><th className="py-6 px-4 font-normal w-1/2">Propuesta</th><th className="py-6 pl-4 font-normal text-right">Pág.</th></tr></thead><tbody className="divide-y divide-white/5">{evidencias.map((item, index) => <tr key={`${item.id}-${index}`} className="group hover:bg-white/[0.02] transition-colors"><td className="py-6 pr-4 text-white font-medium align-top">{item.candidato}</td><td className="py-6 px-4 text-gray-400 font-light text-sm leading-relaxed align-top">{proposal(item)}</td><td className="py-6 pl-4 text-gray-500 font-mono text-[10px] align-top text-right uppercase tracking-widest">{item.pagina_o_seccion ?? "s/d"}</td></tr>)}</tbody></table></div>}</div>}
        </aside>
      </section>
    </div>
  </main>;
